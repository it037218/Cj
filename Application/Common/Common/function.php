 ...
 * */
function get_wx_id(){
    $data = array("user_openid"=>"","nickname"=>"","headimgurl"=>"");
...
function getUserInfo($user_openid){
    $access_token = get_access_token();

    $url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$access_token."&openid=".$user_openid."&lang=zh_CN";
...
/*
 * 根据用户openid 判断用户是否已经存在
 * 没有存在则添加用户信息
 * */
function check_register($data){
    $m = M("cj_member");
    if($data['user_openid'] == -1) {return;};
    $result = $m->where("user_openid = '".$data['user_openid']."'")->find();
    if(!$result){
        //获取用户基本信息
        $info = getUserInfo($data['user_openid']);
        $data['city'] = $info['city'];
...
//    $info['appid'] = "wxe10807d387b1130d";
//    $info['secret'] = "59fa9b340057f86da6d99fc031147127";
...
//	$info = get_token_appinfo ();
    ...
    //配置公众号信息
    $info['appid'] = $rst['appid'];
    $info['secret'] = $rst['app_secret'];
    $info['type'] = 2;
	...
	if (! isset ( $_GET ['getOpenId'] )) {
        ...
        //获取用户信息
        $get_userinfo = "https://api.weixin.qq.com/sns/userinfo?access_token=".$content['access_token']."&openid=".$content['openid']."&lang=zh_CN";
        $userinfo = file_get_contents ( $get_userinfo );
		$userinfo = json_decode ( $userinfo, true );
        $url = $callback . '&openid=' . $userinfo ['openid']."&nickname=".$userinfo['nickname']."&headimgurl=".$userinfo['headimgurl'];
        redirect ( $url );
	}
}
/**
...
function sendKeyMessage($data)
{
    ...
    $param = array(
        ...
        "template_id"=>"ZDmXsOWFtsgEwSz-bpK6Rcw0_0s_y7r9UuenIpiAQxQ",
        "url" => $data['url'],
        "topcolor" => "#FF0000",
        "data" => array(
            "first" => array(
            ...
            "number" => array("value"=>$data['number']),
            "creditName" => array("value"=>$data['creditName']),
            "amount" =>array("value"=>$data['amount']),
    ...
    $msg['remark'] =$data['type'].$data['creditChange'].$data['number'];
...
function sendNoticeMessage($data){
    ...
    $param = array(
        "touser" => $user_openid,
        "template_id"=>"RzlJZ-uELP_t33yLJ1xj0U2PqKr4OyfwDCMUwLe6ThI",
        "url" => $data['url'],
        "topcolor" => "#FF0000",
        "data" => array(
            "first" => array(
    ...
    //注册成功时，发送模板消息提示审核
    function sendVerifyMessage($data){
        $access_token = get_access_token();

        $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" . $access_token;
        $data['first'] = "有新用户注册！[".$data['username']."]赶快去查看吧";
        $data['keyword1'] = "有新用户注册！";
        $data['keyword2'] = "admin";
        $data['keyword3'] = date("Y-m-d H:i:s",time());
        $param = array(
            "touser" => "o0nM4txt6C1J0s-QYOPu8utZZ__I",
            "template_id"=>"YLsKVjRQQCGAj4okj3CAdOcqBaJRdF_HPJgOzvKHOjg",
            "url" => "http://mp.chengjiao8.cn/Admin/index.html",
            "topcolor" => "#FF0000",
            "data" => array(
                "first" => array(
                    "value" => $data['first'],
    ...
    function sendBrandNotice($data){
        $access_token = get_access_token();

        $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" . $access_token;
        $data['first'] = "亲，有一条".$data['brand']."客户信息，赶紧抢啊～别发呆！";
        $data['keyword1'] = "赶紧去查看跟您相关的线索吧！";
        $data['keyword2'] = "橙蕉";
        $data['keyword3'] = date("Y-m-d H:i:s",time());
        $param = array(
            "touser" => $data['user_openid'],
            "template_id"=>"YLsKVjRQQCGAj4okj3CAdOcqBaJRdF_HPJgOzvKHOjg",
            "url" => "http://mp.chengjiao8.cn/index.php?s=/addon/Chengjiaojia/Key/index",
            "topcolor" => "#FF0000",
            "data" => array(
                "first" => array(
                    "value" => $data['first'],
	...
	//橙蕉豆变化
    function changeAccount($condition,$data){
//		var_dump($condition);exit;
        $result1 = M("cj_member")->where($condition)->setInc("use_account",$data['use_account']);
        $result2 = M("cj_member")->where($condition)->setDec("frozen_account",$data['frozen_account']);
//		var_dump($result1);exit;
    ...
    //积分变化
    function changeCredit($condition,$data){
        $result1 = M("cj_member")->where($condition)->setInc("use_credit",$data['use_credit']);
        $result2 = M("cj_member")->where($condition)->setDec("frozen_credit",$data['frozen_credit']);
        if($result1>0 || $result2>0){
            return true;
        }
    }
    //经验值变化
    function changeJifen($condition,$data){
        M("cj_member")->where($condition)->setInc("jifen",$data);

        //获取当前经验值
        $jifen = M("cj_member")->where($condition)->find();
        $com = $jifen['jifen']/100;
        if(ceil($com) > $jifen['level']){
            $level['level'] = ceil($com);
            M("cj_member")->where($condition)->save($level);
        }else if( $com == $jifen['level'] ){
            $level['level'] = ceil($com) + 1;
            M("cj_member")->where($condition)->save($level);
        }
    }
    ...
    function addSysMsg($data){
        if(!empty($data)){
            return M("cj_message")->add($data);
        }
    }

    //添加积分明细
    function addCreditLog($data){
        return M("cj_credit_log")->add($data);
    }
    //添加橙蕉豆明细
    function addAccountLog($data){
        return M("cj_account")->add($data);
    }
	...
	function getQRcode($avatar,$user_openid,$scene_id){
		...
		$tick = urlencode($result['ticket']);
		$url= "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$tick;

		$result = file_get_contents($url);

		$ImgFilePath = $_SERVER['DOCUMENT_ROOT'] . "/Uploads/QR";
		...
		$pos = stripos($avatar,"/");
		if($pos == 0){
			$avatar = "http://mp.chengjiao8.cn".$avatar;
		}

		//二维码
		$QR = imagecreatefromstring($result);
		//中间头像
...
//		var_dump($logo);exit;
		...
		$ImgFileName = $user_openid.".png";
		$ImgFilePath .= ("/" . $ImgFileName);
		imagepng($QR, $ImgFilePath);
		$result = "/Uploads/QR/".$ImgFileName;
		return $result;
	}
